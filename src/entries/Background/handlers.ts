...
export const onBeforeRequestStoreBody = (
  ...
  const uploadData: browser.WebRequest.UploadData[] | undefined =
  ...
  if (uploadData !== null && (uploadData?.length ?? 0) > 0) {
    ...
    const body: string = decodeURIComponent(
      String.fromCharCode.apply(null, rawBody),
    );
...
export const onBeforeHeadersAutoNotarize = (
  ...
  autoNotarizeRequest(req).finally(
    () => delete autoNotarizeRequestBodies[req.requestId],
  );
};
...
const shouldAutoCapture = (
  ...
  return (
    ...
    details.url.includes('://api.twitter.com/1.1/account/settings.json')
  );
};
